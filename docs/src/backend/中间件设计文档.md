...
2. CORS中间件 (cors)
3. 日志中间件 (logger)：使用 process.stdout/stderr 结构化输出，避免 console
4. 速率限制中间件 (rateLimit)
5. 认证中间件 (jwtAuth)
...
处理未找到的路由，返回标准化的404响应；错误日志采用 stderr 写入
...
自动屏蔽敏感字段如：password, token, authorization；统一输出格式 `request_start/request_done/request_error/access` 等标签
...
- **documentUpload**：文档上传（支持pdf, doc, docx, txt）；URL 前缀与目录由 `uploadDir` 控制，支持保留原始文件名
...
```mermaid
flowchart TD
  A[进入路由] --> B{excludePaths?}
  B -- 是 --> N[next]
  B -- 否 --> C{Authorization 是否为 Bearer}
  C -- 否 --> D[401]
  C -- 是 --> E[verify JWT]
  E -- 失败 --> D
  E -- 成功 --> F[User.findByPk]
  F -- 不存在 --> D
  F -- 存在 --> G[UserRole→Role 获取角色代码]
  G --> H{roles 约束满足?}
  H -- 否 --> I[403]
  H -- 是 --> J{permissions 约束?}
  J -- 满足/空 --> K[ctx.userId/state.userId 赋值]
  J -- 不满足 --> I
  K --> N[next]
```
...
sequenceDiagram
  ...
  MW->>MW: request_start 写入 stdout
  MW->>Handler: await next()
  alt 正常响应
    MW->>MW: request_done 写入 stdout
  else 异常
    MW->>MW: request_error 写入 stderr
  end
  MW->>MW: access/server_error/client_error 汇总
```
